...
- Dispatch value = questionsResolved + questionsAdded + branchesKilled; findings are support for movement
...
## Running
- **Codex sandbox**: `npx tsx` spawns esbuild subprocesses that Codex's sandbox blocks. Always use the pre-compiled JS: run `npm run build` first, then `node dist/cli.js` instead of `npx tsx src/cli.ts`. Example: `node dist/cli.js conduct <project> --provider codex`
