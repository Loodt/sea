...
## State
- Conductor version: v053 (iter 44-53 confirmed elite closure persists: zero attrition, perfect persistence, mechanism and kill-check are the most reliable branch closers, design-space works when tied to an immediate live decision, warm-branch synthesis/design-space repetition now looks like the main closure leak, and metric semantics drift is visible whenever `newQuestionsCreated` goes negative.)
...
**Metric drift fallback:** If any of the last 10 metric events has negative `newQuestionsCreated`, `EVENT_ID_COLLISION`, or `METRIC_SEMANTICS_DRIFT`, treat creation metrics as untrusted for selection and evolution. Compute pressure from the live question store only, ignore net-open inferences from metrics, and do not reward a branch for apparent negative creation.
...
**Answered-ID finality:** An `answered` question ID is terminal for dispatch. If follow-up work remains on the branch, reopen only with a fresh dependent question ID or an explicitly logged blocker; never continue the same question via another synthesis/design-space pass.
...
- **Metric quarantine gate:** Any negative `newQuestionsCreated` immediately logs `METRIC_SEMANTICS_DRIFT`, quarantines creation metrics from selector/evolve decisions, and requires `grossQuestionsCreated` plus `openQuestionsDelta` separation before those fields can influence scoring or pressure modes again.
...
2. **Metric semantics & event identity** (HIGH) - metrics need durable dispatch event IDs plus a clean split between `grossQuestionsCreated`, `openQuestionsDelta`, and closure counts so resumed runs, crash backfills, and same-iteration multi-dispatch cases cannot collide or report negative `newQuestionsCreated`.
3. **Early-exit & observability completion** (MEDIUM) - zero-finding searches at inner iter 2 still need code-level convergence control, and observability must emit `PERSISTENCE_GAP`, `HOLLOW_ANSWER`, `LOW_VERIFICATION_COMPLETION`, `DISPATCH_GAP`, `EXHAUSTED_UNRESOLVED`, `NON_CLOSING_ANSWER`, `THIN_CLOSURE`, `DESIGN_SPACE_OVERHANG`, `HARVEST_DRIFT`, `SYNTHESIS_SPIN`, `EVENT_ID_COLLISION`, and `METRIC_SEMANTICS_DRIFT` consistently, including metric-quarantine state.
