...
2. **Observability** (MEDIUM) — PERSISTENCE_GAP, HOLLOW_ANSWER, DISPATCH_GAP, EXHAUSTED_UNRESOLVED, MANDATE_SKIPPED (new v057): emit consistently. `findingsPersisted` records pre-restore delta on auto-restore (iter 18 LQ031 reported -343 while store grew normally) — masks STORE_CLOBBER_RESTORED in metrics dashboards.
3. **Mandate enforcement** (BLOCKING — v057 escalation) — 6 prompt-only attempts (v051–v056) failed to force FP/mechanism/design-space creation+dispatch across 29 local-llm-stack iters. v057 demoted mandates to priors with `MANDATE_SKIPPED` logging. Real fix requires code: (a) pre-rank mandate-eligibility check, (b) auto-create missing-type question if 0 open of mandated type, (c) optional hard-block on non-mandated dispatches behind feature flag. **Do NOT add more prompt-only mandate prose** — further revisions are infrastructure work, not conductor evolution.
