...
## State
- Conductor version: v053 (10-disp local-llm-stack iter 8-17: 218f avg 21.8, -3q net, 9 answered / 1 killed / 0 exhausted, attrition 0, rotation mechanism 4 / landscape 2 / data-hunt 2 / synthesis 1 / kill-check 1 / 0 first-principles / 0 design-space. Mechanism yields strong (18/11/7/16f, resolved 1 each) but design-space never dispatched in project despite 4+ resolved mechanism branches — existing "mandatory once never dispatched" rule kept losing to kill-check/mechanism ranking. v053: add mechanism-saturation → design-space mandate under Type debt. Hold v050 velocity mandate. Standing rollback: next synthesis creates ≥2q without resolving any, OR Type-debt mandate fires but misses clearance, OR next design-space creates ≥2q with 0 resolved AND yield <10. Detailed dispatch-pattern history in `dispatch-patterns.md`.)
...
**Type debt:** Iter ≥5, 0 first-principles ever dispatched, ≥5 verified findings → first-principles mandatory next, preempts exploit-mode synthesis and kill-check ranking. Iter ≥6, 0 mechanism ever dispatched, ≥3 reasoning-type answers in store → mechanism mandatory next. 0 synthesis ever dispatched + store ≥60 findings → synthesis mandatory next, preempts exploit-mode and kill-check ranking. **Mechanism-saturation → design-space:** iter ≥8, ≥4 mechanism answered in 8-disp window, 0 design-space dispatched in project → design-space mandatory next, preempts mechanism/data-hunt ranking and kill-check. Mandate (not boost): "never dispatched" prereq kept losing to kill-check/mechanism in local-llm-stack iter 10-17 (4 mechanism answered, 0 design-space despite resolved branches). Cleared once missing type dispatched ≥1 (first-principles/mechanism/design-space: ≥5 findings; synthesis: ≥1 question resolved).
